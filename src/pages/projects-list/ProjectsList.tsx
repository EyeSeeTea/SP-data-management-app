...
const Link: React.FC<{ url: string }> = ({ url }) => {
  return (
    <a rel="noopener noreferrer" style={{ wordBreak: "break-all", textDecoration: "none" }} href={url} target="_blank">
      {url}
    </a>
  );
};
...
function getConfig(history: History, currentUser: CurrentUser, setTargetPopulation: React.Dispatch<React.SetStateAction<boolean>>) {
  const columns = [{ name: "displayName", text: i18n.t("Name"), sortable: true }, { name: "publicAccess", text: i18n.t("Public access"), sortable: true }, { name: "lastUpdated", text: i18n.t("Last updated"), sortable: true }];
  ...
  const allActions = {
    ...
    dataEntry: {
      ...
      multiple: false,
      onClick: (dataSet: DataSet) => history.push(generateUrl("dataEntry.edit", { id: dataSet.id }))
    },
    ...
    edit: {
      ...
      text: i18n.t("Edit"),
      multiple: false,
      onClick: (dataSet: DataSet) => history.push(generateUrl("projects.edit", { id: dataSet.id }))
    },
...
const ProjectsList: React.FC = () => {
  ...
  const list = (_d2: unknown, filters: FiltersForList, pagination: Pagination) => Project.getList(api, config, filters, pagination);

  const newProjectPageHandler = currentUser.canCreateProject() ? () => goTo(history, generateUrl("projects.new")) : null;

  return (
    <React.Fragment>
      <PageHeader title={i18n.t("Projects")} help={componentConfig.help} onBackClick={goToLandingPage} />

      <OldObjectsTable model={{ modelValidations: {} }} columns={componentConfig.columns} d2={{}} detailsFields={componentConfig.detailsFields} initialSorting={componentConfig.initialSorting} pageSize={20} actions={componentConfig.actions} list={list} disableMultiplePageSelection={true} buttonLabel={i18n.t("Create Project")} onButtonClick={newProjectPageHandler} />
